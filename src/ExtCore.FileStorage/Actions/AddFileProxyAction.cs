...
namespace ExtCore.FileStorage.Actions
{
  ...
  /// </summary>
  public class AddFileProxyAction : IConfigureServicesAction
  {
    /// <summary>
    ...
    {
      Type type = ExtensionManager.GetImplementations<IFileProxy>()?.FirstOrDefault(t => !t.GetTypeInfo().IsAbstract);

      if (type == null)
      {
        ILogger logger = serviceProvider.GetService<ILoggerFactory>().CreateLogger("ExtCore.FileStorage");

        logger.LogError("Implementation of ExtCore.FileStorage.Abstractions.IFileProxy not found");
        return;
      }

      services.AddScoped(typeof(IFileProxy), type);
    }
  }
}